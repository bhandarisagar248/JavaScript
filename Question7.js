// The Local Storage Manager: You are working on a note-taking app, and you want to implement a function named saveNoteToLocalStorage that takes a note object and saves it to the browser's local storage.


//declearing function 
function saveNoteToLocalStorage(obj){

//converting the object into String using JSON Stringify() method
const Note=JSON.stringify(obj);

// adding note to the local storage
localStorage.setItem("note",Note);

//writing the stored note to the html element
document.getElementById("output").innerHTML=localStorage.getItem("note");
    }
    




//creating an object for taking note of items
const note={
    1:"Rice",
    2:"Daal",
    3:"Tea Leaves",
    4:"Sugar",
    5:"Salt",
    6:"Sabji Masala",
}


// calling function
saveNoteToLocalStorage(note);